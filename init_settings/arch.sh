...
install_packages_aur=()         # Because you can't install all packages you need by makepkg, declare an array, then iterate.
while read install_code
do

    method_name=$(echo $install_code|awk '{print $1}')
